...
export type UserRole =
  ...
  | 'eventos_patrimonio'
  | 'visualizador'
  // Papéis de compatibilidade mantidos
  | 'tesouraria'
  | 'eventos'
  | 'diretor_eventos'
  | 'comissao_formatura'
...
export interface User {
  id: string;
  name: string;
  warName: string; // Nome de Guerra / Posto (ex: Ten. R/2 Felipe, AL PONCE, AL EMANUEL)
  email: string;
  role: UserRole;
  status?: UserStatus;
  avatar?: string;
...
export interface AppThemeConfig {
  mode: 'light' | 'dark' | 'tactical' | 'olive';
  primaryColor: string; // Cor primária (ex: '#4B5320')
  accentColor: string;  // Cor de destaque/ouro (ex: '#D4AF37')
  headerStyle: 'light' | 'dark' | 'primary';
  sidebarStyle?: 'dark' | 'primary' | 'olive';
  customLogoUrl?: string; // Imagem/Emblema personalizado carregado pelo admin
  customLogoName?: string;
  fontStyle?: 'institutional' | 'modern';
  borderRadius?: 'none' | 'sm' | 'md';
  showBackgroundWatermark?: boolean;
  watermarkOpacity?: number; // 0 to 20
...
export interface GremioConfig {
  ...
  /** Saldo atual explicitamente informado pela PLANILHA MÃE. Quando presente, tem precedência sobre saldo calculado. */
  currentCashBalance?: number;
  /** Totais consolidados reais da aba Resumo da PLANILHA MÃE. */
  totalRealRevenue?: number;
  totalRealExpenses?: number;
  totalPlannedRevenue?: number;
  totalPlannedExpenses?: number;
  financialSummarySource?: string;
...
export interface Transaction {
  ...
  origin?: 'Google Drive' | 'Manual' | 'Planilha Importada';
  sourceFileId?: string;
  ...
  /** True para linhas de resumo/detalhamento que não devem ser somadas novamente em KPIs consolidados. */
  excludedFromFinancialTotals?: boolean;
...
export interface MonthlyFee {
  ...
  amount: number;
  paymentDate?: string;
  status: 'Pago' | 'Pendente' | 'Atrasado' | 'Isento' | 'Parcial' | 'Não aplicável';
...
export interface DocumentRecord {
  ...
  driveFolderPath?: string;
  mimeType?: string;
  financialDirection?: 'entrada' | 'saida';
  originalFileUrl?: string;
  sourceFileId?: string;
  fileId?: string;
  modifiedTime?: string;
  ...
  isDemoData?: boolean;
  // Classificação Automática & Inteligência Contábil
  categoryId?: string;
  categoryName?: string;
  suggestedCategoryId?: string;
  suggestedCategoryName?: string;
  classificationConfidence?: number; // 0.0 a 1.0 (>= 0.90 classifica direto; < 0.90 aguarda confirmação)
  classificationStatus?: 'classificado' | 'aguardando_confirmacao' | 'aguardando_classificacao';
  suggestedTransactionId?: string;
  transactionLinkConfidence?: number; // 0.0 a 1.0 (>= 0.85 vincula direto; 0.50 a 0.84 aguarda confirmação)
  transactionLinkStatus?: 'vinculado' | 'aguardando_confirmacao' | 'sem_vinculo';
  ...
  ocrStatus?: 'pending' | 'processing' | 'success' | 'failed' | 'needs_review';
  ocrProgress?: number;
  ocrError?: string;
...
export interface Category {
  ...
  source: 'predefined' | 'automatic' | 'user_created';
  isDynamic: boolean;
  createdAt: string;
  description?: string;
  color?: string;
...
export interface ClassificationRule {
  ...
  priority: number;
  createdAt: string;
  isUserConfirmed: boolean; // criado automaticamente ou confirmado pelo usuário
}
...
export interface SyncSummaryReport {
  ...
  documentsUnidentified: number;
  automaticLinks: number;
  linksPendingConfirmation: number;
  pendingConfirmations?: number;
  newSuppliersIdentified?: number;
  existingCategoriesUsed: string[];
