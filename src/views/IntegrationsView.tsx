...
import { AppState, formatDatePtBr, saveTransactionToFirestore } from '../services/dataService';
import {
  OFFICIAL_CATEGORIES,
  ROOT_FOLDER_ID,
  ROOT_FOLDER_URL,
  ...
  const handleConnectGoogle = async () => {
    try {
      ...
      if (res) {
        ...
        setSyncFeedback({
          type: 'success',
          message: `Conectado com sucesso à conta Google (${res.user.email || res.user.displayName}). Iniciando esteira de sincronização e lançamento automático dos documentos...`,
        });
        // Disparo automático do fluxo assim que a conexão for concluída:
        // O usuário não precisa cadastrar manualmente aquilo que já existe no Google Drive ou na PLANILHA MÃE.
        setTimeout(() => {
          handleTriggerSync();
        }, 300);
      }
    ...
    try {
      ...
      if (res.ok && data.success) {
        if (Array.isArray(data.transactions) && data.transactions.length > 0) {
          await Promise.all(
            data.transactions.map((t: any) => saveTransactionToFirestore(t).catch(console.warn))
          );
        }
        setImportStatus(`Sucesso: ${data.importedCount} registros importados para "${selectedFolder}" e salvos no Cloud Firestore. ${data.duplicatedCount} duplicidades evitadas.`);
        setImportText('');
      } else {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12 text-slate-800">
      ...
      <div className="bg-white border border-slate-200 p-6 shadow-sm space-y-4">
        ...
        {/* Automated Pipeline Visual Status */}
        <div className="bg-[#1A2421]/5 border border-[#4B5320]/20 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-[#4B5320]" />
              <h4 className="text-xs font-bold uppercase tracking-wider text-[#1A2421]">
                Esteira de Lançamento e Conciliação Contábil Automática
              </h4>
            </div>
            {isSyncing ? (
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-blue-100 text-blue-800 text-[10px] font-bold border border-blue-300 animate-pulse">
                <RefreshCw className="w-3 h-3 animate-spin" />
                Executando Etapas...
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-100 text-emerald-800 text-[10px] font-bold border border-emerald-300">
                <CheckCircle2 className="w-3 h-3" />
                Fluxo Ativo
              </span>
            )}
          ...
          <p className="text-[11px] text-slate-600 leading-relaxed">
            Assim que o Google Drive é conectado, o sistema executa automaticamente sem intervenção manual:
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 text-[10px]">
            <div className="p-2 bg-white border border-slate-200 flex items-start gap-2">
              <span className="w-4 h-4 bg-[#4B5320] text-white rounded-full flex items-center justify-center font-bold text-[9px] shrink-0">1</span>
              <div>
                <strong className="text-slate-800 block">Varredura Recursiva</strong>
                <span className="text-slate-500">Lê a pasta raiz do Grêmio e todas as subpastas</span>
              </div>
            </div>
            <div className="p-2 bg-white border border-slate-200 flex items-start gap-2">
              <span className="w-4 h-4 bg-[#4B5320] text-white rounded-full flex items-center justify-center font-bold text-[9px] shrink-0">2</span>
              <div>
                <strong className="text-slate-800 block">Leitura da PLANILHA MÃE</strong>
                <span className="text-slate-500">Extrai lançamentos, saldo inicial e abas</span>
              </div>
            </div>
            <div className="p-2 bg-white border border-slate-200 flex items-start gap-2">
              <span className="w-4 h-4 bg-[#4B5320] text-white rounded-full flex items-center justify-center font-bold text-[9px] shrink-0">3</span>
              <div>
                <strong className="text-slate-800 block">Extração de Evidências</strong>
                <span className="text-slate-500">Identifica valor, data, fornecedor e finalidade</span>
              </div>
            </div>
            <div className="p-2 bg-white border border-slate-200 flex items-start gap-2">
              <span className="w-4 h-4 bg-[#4B5320] text-white rounded-full flex items-center justify-center font-bold text-[9px] shrink-0">4</span>
              <div>
                <strong className="text-slate-800 block">Lançamento Automático</strong>
                <span className="text-slate-500">Cria ou atualiza movimentações sem digitação duplicada</span>
              </div>
            </div>
            <div className="p-2 bg-white border border-slate-200 flex items-start gap-2">
              <span className="w-4 h-4 bg-[#4B5320] text-white rounded-full flex items-center justify-center font-bold text-[9px] shrink-0">5</span>
              <div>
                <strong className="text-slate-800 block">Vinculação de Comprovantes</strong>
                <span className="text-slate-500">Associa comprovantes fiscais às suas movimentações</span>
              </div>
            </div>
            <div className="p-2 bg-white border border-slate-200 flex items-start gap-2">
              <span className="w-4 h-4 bg-[#4B5320] text-white rounded-full flex items-center justify-center font-bold text-[9px] shrink-0">6</span>
              <div>
                <strong className="text-slate-800 block">Categorias Dinâmicas</strong>
                <span className="text-slate-500">Cria novas páginas (ex: Material Ponte) automaticamente</span>
              </div>
            </div>
          </div>
        </div>
