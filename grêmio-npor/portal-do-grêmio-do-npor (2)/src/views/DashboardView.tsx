...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const currentBalance = (config.initialBalance || 0) + totalRevenues - totalPaidExpenses;
      ...
      <div className="bg-[#1A2421] text-white p-6 border-l-4 border-[#D4AF37] shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            {config.unitName} • Presidente: {config.presidentName || 'Felipe Gonçalves Ponce'}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 border-b border-slate-100 pb-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={handleSync}
              disabled={isSyncing}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#4B5320] hover:bg-[#3b4119] disabled:opacity-50 text-white text-xs font-bold uppercase tracking-wider transition shadow-sm"
