...
export const ReportsView: React.FC<ReportsViewProps> = ({ appState, userRole }) => {
  ...
  const computedTotalExpenses = transactions
  ...
  const totalExpenses = typeof config.totalRealExpenses === 'number' ? config.totalRealExpenses : computedTotalExpenses;
  const finalBalance = typeof config.currentCashBalance === 'number'
    ? config.currentCashBalance
    : config.initialBalance + totalRevenues - totalExpenses;
        ...
        <div className="pt-12 border-t border-stone-800 print:border-stone-400 grid grid-cols-1 sm:grid-cols-3 gap-8 text-center text-xs">
          <div className="space-y-1">
            ...
            <p className="font-bold text-white print:text-black">
              {appState.commissionMembers?.find((m) => m.role === 'presidente')?.name || 'AL PONCE'}
            </p>
          ...
          <div className="space-y-1">
            <div className="border-b border-stone-600 print:border-black w-4/5 mx-auto h-8" />
            <p className="font-bold text-white print:text-black">
              {appState.commissionMembers?.find((m) => m.role === 'tesoureiro_1' || m.role === 'tesouraria')?.name || 'AL EMANUEL'}
            </p>
            <p className="text-[10px] text-stone-400 print:text-stone-600">1º Tesoureiro / Diretor Financeiro</p>
          </div>

          <div className="space-y-1">
            <div className="border-b border-stone-600 print:border-black w-4/5 mx-auto h-8" />
            <p className="font-bold text-white print:text-black">
              {appState.commissionMembers?.find((m) => m.role === 'vice_presidente')?.name || 'AL ARAUJO'}
            </p>
            <p className="text-[10px] text-stone-400 print:text-stone-600">Vice-Presidente do Grêmio do NPOR</p>
          </div>
        </div>
      </div>
    </div>
